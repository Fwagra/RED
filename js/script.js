...
function toggleFolder(element,folder){
	...
	$(element).html((!open?'<span class="deplier"></span>':'<span class="plier"></span>'));
	$.ajax({
...
function addFavorite(element,id){
	...
	$(element).attr('onclick','removeFavorite(this,'+id+');').html('<div class="unfavtemp" >Défavoriser</div>');
	$(element).className = "truc";
...
function removeFavorite(element,id){
	$(element).attr('onclick','addFavorite(this,'+id+');').html('<div class="favtemp" >Favoriser</div>');
	$.ajax({
